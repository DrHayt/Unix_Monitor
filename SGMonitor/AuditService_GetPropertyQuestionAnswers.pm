use strict;
use warnings;
package SGMonitor::AuditService_GetPropertyQuestionAnswers;;
...
sub new(){
    ...
    $self->{SERVICE_NAME}="AuditService.GetPropertyQuestionAnswers";
    $self->{BASE_STRING}="ServiceBus.monitor." . uc($self->{SERVICE_NAME});

    $self->{RANGE}=60000000;
    $self->{START}=100000000;
    $self->{SPIPROPERTYID}=$args->{SPIPROPERTYID} || int(rand($self->{RANGE}))+$self->{START};
...
sub run(){
    my $self=shift;
    my $id=int($self->{SPIPROPERTYID});

    my %params=( 'spiPropertyId' => int($id) );
